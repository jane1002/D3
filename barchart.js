var LDAvis = function(to_select, data_or_file_name) {
  ...
  function visualize(data) {
    ...
    function reorder_bars(increase) {
      console.log('in reorder_bars, this will select the TOP 30 terms which is related to the topic');
      ...
      console.log('before sort dat2', dat2);

      // sort by relevance:
      dat2.sort(fancysort("relevance"));

      console.log('after sort dat2', dat2);

      // truncate to the top R tokens:
      var dat3 = dat2.slice(0, R);

      console.log('after select R from dat2 -> dat3', dat3);
...
d3.json("static/movie_prepare_data",function(error, data) {
